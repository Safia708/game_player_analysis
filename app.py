...
if show_mapping_ui:
    ...
    if not apply_clicked:
        if unmapped:
            st.info(f"Still unmapped (will be skipped): {', '.join(f'`{c}`' for c in unmapped)}")
        st.stop()

    rename_map = {
        src: rc for rc, src in st.session_state[mapping_key].items() if src != "-- None --"
    }
    mapped_required = [rc for rc in REQUIRED_COLS if rc in rename_map.values()]
    df_raw = df_uploaded.rename(columns=rename_map)[mapped_required + [
        c for c in df_uploaded.columns
        if c not in rename_map and c not in REQUIRED_COLS
    ]]
    if unmapped:
        st.warning(
            "Running with a partial format. These fields have no equivalent in your file, "
            "so any analysis that depends on them will be skipped below: "
...
# skip only the specific analyses that need them.
missing_cols = set(c for c in REQUIRED_COLS if c not in df_raw.columns)

if missing_cols:
    st.sidebar.warning(
        "Skipping analyses that need: " + ", ".join(f"`{c}`" for c in sorted(missing_cols))
    )

# ----------------------------------------------------------------------
# Which columns each analysis needs, so we can skip just that analysis
# (not the whole dashboard) when a field truly isn't in the user's data.
# ----------------------------------------------------------------------
SECTION_REQS = {
    "churn": {"age", "sessions_per_week", "avg_session_minutes", "playtime_hours_total",
              "player_level", "achievements_unlocked", "matches_played", "win_ratio",
              "kd_ratio", "engagement_level", "player_id", "game_genre"},
    "loyalty": {"sessions_per_week", "avg_session_minutes", "playtime_hours_total",
                "player_level", "achievements_unlocked", "matches_played",
                "engagement_level", "player_id", "game_genre"},
    "spend": {"in_game_purchases", "engagement_level", "playtime_hours_total",
              "purchase_amount_usd", "sessions_per_week"},
    "genre": {"game_genre", "playtime_hours_total", "sessions_per_week",
              "avg_session_minutes", "engagement_level", "player_id"},
    "difficulty": {"difficulty_preference", "playtime_hours_total", "sessions_per_week",
                   "win_ratio", "engagement_level", "player_id"},
    "cluster": {"sessions_per_week", "avg_session_minutes", "playtime_hours_total",
                "player_level", "win_ratio", "purchase_amount_usd", "age"},
    "predict": {"gender", "game_genre", "difficulty_preference", "age",
                "sessions_per_week", "avg_session_minutes", "playtime_hours_total",
                "purchase_amount_usd"},
    "skill": {"playtime_hours_total", "win_ratio", "kd_ratio", "player_level"},
}
# spend also needs loyalty_score, which only exists if "loyalty" ran
SECTION_DEPENDS_ON = {"spend": ["loyalty"]}


def section_available(name: str) -> bool:
    return not (SECTION_REQS[name] & missing_cols)


def missing_for(name: str) -> list:
    return sorted(SECTION_REQS[name] & missing_cols)
...
def run_pipeline(df_in: pd.DataFrame, available_sections: dict):
    df = df_in.copy()
    artifacts = {name: None for name in SECTION_REQS}

    # ---------- 2. Churn risk ----------
    if available_sections["churn"]:
        feature_cols = [
    ...
    # ---------- 4. Spend vs loyalty ----------
    if available_sections["spend"] and "loyalty_score" in df.columns:
    ...
    # ---------- 6. Difficulty ----------
    if available_sections["difficulty"]:
...
_available_sections = {name: section_available(name) for name in SECTION_REQS}
df, art = run_pipeline(df_raw, _available_sections)
...
k2.metric("Avg. playtime", f"{df['playtime_hours_total'].mean():.0f}h" if "playtime_hours_total" in df else "—")
k3.metric("Avg. sessions/wk", f"{df['sessions_per_week'].mean():.1f}" if "sessions_per_week" in df else "—")
k4.metric("High engagement", f"{(df['engagement_level'] == 'High').mean() * 100:.1f}%" if "engagement_level" in df else "—")
k5.metric("Payer rate", f"{df['in_game_purchases'].mean() * 100:.1f}%" if "in_game_purchases" in df else "—")
k6.metric("Total revenue", f"${df['purchase_amount_usd'].sum():,.0f}" if "purchase_amount_usd" in df else "—")

if art["churn"] is not None:
    st.markdown(
        f"🚩 **{art['churn']['n_flight_risk']}** Medium-engagement players are showing "
        "Low-engagement behavior (flight risk)."
    )
else:
    st.caption(f"Flight-risk summary skipped — needs: {', '.join(f'`{c}`' for c in missing_for('churn'))}")

dc1, dc2, dc3 = st.columns(3)

with dc1:
    if "engagement_level" in df:
        ...
        st.info("Engagement chart skipped — needs `engagement_level`.")
...
with dc2:
    if "game_genre" in df:
        ...
        st.info("Genre chart skipped — needs `game_genre`.")
...
with dc3:
    if "cluster" in df:
        ...
        st.info(f"Segment chart skipped — needs: {', '.join(f'`{c}`' for c in missing_for('cluster'))}")

dc4, dc5 = st.columns(2)
with dc4:
    if "playtime_hours_total" in df:
        ...
        st.info("Playtime chart skipped — needs `playtime_hours_total`.")

with dc5:
    if {"engagement_level", "purchase_amount_usd"} <= set(df.columns):
        fig, ax = plt.subplots(figsize=(6, 4))
        ...
        st.info("Revenue-by-engagement chart skipped — needs `engagement_level` and `purchase_amount_usd`.")
...
# ---------------- Churn ----------------
with tabs[1]:
    if art["churn"] is None:
        st.info(
            "🚫 Skipped — churn-risk scoring needs: "
            + ", ".join(f"`{col}`" for col in missing_for("churn"))
        )
    else:
        c = art["churn"]
...
# ---------------- Super fans ----------------
with tabs[2]:
    if art["loyalty"] is None:
        st.info(
            "🚫 Skipped — the loyalty score needs: "
            + ", ".join(f"`{col}`" for col in missing_for("loyalty"))
        )
    else:
        l = art["loyalty"]
...
# ---------------- Spend vs loyalty ----------------
with tabs[3]:
    if art["spend"] is None:
        needed = sorted(set(missing_for("spend")) | (set(missing_for("loyalty")) if art["loyalty"] is None else set()))
        st.info("🚫 Skipped — spend-vs-loyalty needs: " + ", ".join(f"`{col}`" for col in needed))
    else:
        s = art["spend"]
...
# ---------------- Genre ----------------
with tabs[4]:
    if art["genre"] is None:
        st.info(
            "🚫 Skipped — genre analysis needs: "
            + ", ".join(f"`{col}`" for col in missing_for("genre"))
        )
    else:
        g = art["genre"]
...
# ---------------- Difficulty ----------------
with tabs[5]:
    if art["difficulty"] is None:
        st.info(
            "🚫 Skipped — difficulty analysis needs: "
            + ", ".join(f"`{col}`" for col in missing_for("difficulty"))
        )
    else:
        d = art["difficulty"]
...
# ---------------- Segmentation ----------------
with tabs[6]:
    if art["cluster"] is None:
        st.info(
            "🚫 Skipped — segmentation needs: "
            + ", ".join(f"`{col}`" for col in missing_for("cluster"))
        )
    else:
        cl = art["cluster"]
...
# ---------------- Prediction ----------------
with tabs[7]:
    if art["predict"] is None:
        st.info(
            "🚫 Skipped — prediction models need: "
            + ", ".join(f"`{col}`" for col in missing_for("predict"))
        )
    else:
        p = art["predict"]
...
# ---------------- Skill curve ----------------
with tabs[8]:
    if art["skill"] is None:
        st.info(
            "🚫 Skipped — skill-curve analysis needs: "
            + ", ".join(f"`{col}`" for col in missing_for("skill"))
        )
    else:
        sk = art["skill"]
...
with tabs[9]:
    ...
    if c is not None:
        findings.append(f"""**1. Churn risk**
...
- **{c['n_flight_risk']} of {c['n_medium']}** Medium-engagement players already look behaviorally
  like Low-engagement players — that's your win-back list.""")
        ...
        findings.append(f"**1. Churn risk** — skipped, needs: {', '.join(missing_for('churn'))}")

    if l is not None:
        findings.append(f"""**2. Super fans**
- Top loyalty-score players average **{l['super_fans']['sessions_per_week'].mean():.1f}** sessions/week
  ...
  **{df['sessions_per_week'].mean():.1f}** and **{df['playtime_hours_total'].mean():.0f}** for the full base.""")
        ...
        findings.append(f"**2. Super fans** — skipped, needs: {', '.join(missing_for('loyalty'))}")

    if s is not None:
        findings.append(f"""**3. Spend vs. loyalty**
...
- Verdict: this dataset **{s['verdict_spend']}** the "spenders = most engaged" assumption.""")
        ...
        findings.append(f"**3. Spend vs. loyalty** — skipped, needs: {', '.join(missing_for('spend'))}")

    if g is not None:
        findings.append(f"""**4. Genre**
- Stickiest genre: **{g['best_genre']}** · Least sticky: **{g['worst_genre']}**""")
    else:
        findings.append(f"**4. Genre** — skipped, needs: {', '.join(missing_for('genre'))}")

    if d is not None:
        findings.append(f"""**5. Difficulty**
- Spread in High-engagement % across difficulty levels: **{d['spread']:.1f} points**
  (small = difficulty barely matters here; large = it matters a lot)""")
        ...
        findings.append(f"**5. Difficulty** — skipped, needs: {', '.join(missing_for('difficulty'))}")

    if cl is not None:
        findings.append(f"""**6. Segmentation**
- **{cl['K']}** behavioral segments identified via KMeans
  (silhouette={cl['sil_scores'].get(cl['K'], cl['sil_scores'][cl['best_k']]):.3f})""")
    else:
        findings.append(f"**6. Segmentation** — skipped, needs: {', '.join(missing_for('cluster'))}")

    if p is not None:
        findings.append(f"""**7. Prediction**
- Playtime model R²={p['r2_playtime']:.3f}, MAE={p['mae_playtime']:.1f}h
- Spend model R²={p['r2_spend']:.3f}, MAE=${p['mae_spend']:.2f}""")
    else:
        findings.append(f"**7. Prediction** — skipped, needs: {', '.join(missing_for('predict'))}")

    if sk is not None:
        findings.append(f"""**8. Skill vs. time played**
- r(playtime, win_ratio)={sk['r_playtime_win']:+.3f}, r(playtime, kd_ratio)={sk['r_playtime_kd']:+.3f}
- Verdict: {sk['verdict_skill']}""")
    else:
        findings.append(f"**8. Skill vs. time played** — skipped, needs: {', '.join(missing_for('skill'))}")
